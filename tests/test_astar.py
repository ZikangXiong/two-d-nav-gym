...
def define_obstables(board, obs_list):
...
def deconstructPath(path, row, col):
  ...
  for i in range(len(path)-1):
    next = path[i]
    if current.col-c.STEP == next.col:
      actions.append("up")
    elif current.col+c.STEP == next.col:
      actions.append("down")
    elif current.row-c.STEP == next.row:
      actions.append("left")
    elif current.row+c.STEP == next.row:
...
def printActions(actions):
  ...
  for i in range(len(actions)):
    ...
    if current != next or i == len(actions) - 1:
